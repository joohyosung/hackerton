...
- 공공데이터포털에서 제공하는 대출상품 정보를 기반으로 사용자에게 가장 적합한 상품을 추천한다. DSR/DTI/LTV 중심이 아닌 **상품 조건 매칭 기반** 추천 알고리즘을 사용한다.
...
- 상품의 대상(`trgt`, `tgtFltr`, `suprtgtdtlcond`)과 사용자 정보를 비교한다.
...
지자체 상품의 지역이 일치하지 않으면 추천 대상에서 제외한다.
...
다음 조건 중 하나라도 만족하면 추천 대상에서 제외한다.
