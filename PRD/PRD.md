...
### 공공데이터 필드 기반 조건 매칭 알고리즘 적용
- 수정 일시: 2026-06-27
- 기준 문서:
  - `C:/Users/Joo/Desktop/Public_Data_Loan_Scoring_Implementation_Guide.md`
- 변경 목적:
  - 기존 DSR/DTI/LTV 중심 추천식을 제거하고 공공데이터포털 API 응답 필드와 사용자 입력 조건의 일치도를 기준으로 추천 결과를 계산함
  - 사용자가 반드시 입력해야 하는 값을 줄여 검색 진입 장벽을 낮춤
...
| 항목 | 배점 | 설명 |
|---|---:|---|
| 한도 적합도 | 25점 | 희망 대출금액이 상품 한도 안에 들어가는지 평가 |
| 용도 적합도 | 25점 | 사용자 대출 용도와 API `usge` 필드 일치도 평가 |
| 대상 적합도 | 20점 | 사용자 유형과 `trgt`, `tgtFltr`, `suprTgtDtlCond`, 상품구분 텍스트 일치도 평가 |
| 지역 적합도 | 15점 | 거주 지역과 `rsdAreaPamtEqltIstm` 또는 지역 필드 일치도 평가 |
| 조건 적합도 | 10점 | 연령, 소득, 선택 입력된 신용등급 조건 평가 |
| 금리 매력도 | 5점 | API `irt` 필드에서 파싱한 금리 수준 평가 |

#### 백엔드 변경
- GitHub `main`의 분리 구조를 기준으로 로컬 구조를 정리함
  - `LoanApiClient`: 공공데이터포털 API GET 호출 담당
  - `LoanProductXmlParser`: XML 응답을 `LoanProduct`로 변환
  - `LoanRecommendationScorer`: 조건 매칭형 추천 점수 계산 담당
  - `LoanProductService`: API 데이터 로딩, Scorer 호출, 최소 점수 필터링 담당
- `LoanSearchRequest`에서 필수 5개와 선택 2개만 유지함
- `LoanRecommendationScorer` 생성자를 `LoanProductAnalyzer`만 받도록 단순화함
- `LoanProductAnalyzer`의 한도 파싱을 보완해 `5000`처럼 숫자만 들어오는 API 값을 만원 단위로 해석함
- `LoanProductXmlParser`에서 다음 필드 매핑을 보강함
  - `lnLmt`, `lnlmt`
  - `usge`
  - `trgt`, `tgtFltr`, `suprTgtDtlCond`
  - `rsdAreaPamtEqltIstm`, `rsdArea`
  - `instCtg`, `prdCtg`
  - 신청방법, 문의처, 관련 사이트, 상환방법 등 상세 표시 필드
- 최소 추천 점수 기본값을 45점으로 조정함

#### 프론트엔드 변경
- 메인 검색 폼에는 필수 입력값 5개만 바로 노출함
- 신용등급과 사용자 유형은 `상세 조건 입력` 영역으로 접어 선택 입력으로 제공함
- 검색 payload에서 DSR/DTI/LTV 계산용 필드를 제거함
- 결과 카드와 상세 모달에서 DSR/DTI/LTV 수치 대신 조건 매칭 사유와 확인 필요 메시지를 표시함
- 추천 기준 안내 문구를 공공데이터 필드 기반 조건 매칭 기준으로 변경함

#### 테스트 변경
- `LoanRecommendationScorerTest`를 추가함
  - 희망 대출금액이 상품 한도를 초과하면 추천 제외
  - 용도와 지역이 일치하면 높은 점수 반환
  - 지자체 상품의 지역이 불일치하면 추천 제외
- `LoanControllerTest`를 새 입력 구조에 맞게 추가함
  - 신용등급은 선택값이므로 필수 검증 대상에서 제외
- `LoanProductAnalyzerTest`를 보강함
  - 숫자 단독 한도값을 만원 단위로 파싱하는지 검증
- `LoanProductParsingTest`를 보강함
  - 기관구분, 상품구분, 숫자 한도 필드 매핑 검증
