...
본 서비스는 대출 상품 데이터를 다음 순서로 조회합니다.

1. JSON 파일 (`loan.api.json-path`)
2. 공공데이터포털(Open API)
3. 샘플 데이터

### 1. JSON 파일

운영 환경에서는 정규화된 JSON 데이터를 우선 사용합니다.

```text
data/loan-products.json
```

파일이 존재하면 공공데이터 API를 호출하지 않고 JSON 데이터를 사용합니다.

### 2. 공공데이터 API

JSON 파일이 없거나 비어 있는 경우 공공데이터포털 API를 호출합니다.

필요한 설정:

```properties
loan.api.service-key=발급받은_서비스키
```

### 3. 샘플 데이터

다음과 같은 경우에는 샘플 데이터를 사용합니다.

- JSON 파일 없음
- JSON 파일 파싱 실패
- API 호출 실패
- API 응답이 비어 있음
- `loan.api.use-sample-when-unavailable=true`
